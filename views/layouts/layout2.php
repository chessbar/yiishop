    ...
    <div class="container">
        ...
        <div class="col-xs-12 col-sm-6 no-margin">
            <ul class="right">
                ...
                <li>您好，欢迎您回来<?php echo \Yii::$app->session['loginname']?> <a href="<?php echo yii\helpers\Url::to(['member/logout']);?>">退出</a></li>
            <?php else:?>
...
<?php foreach($this->params['menu'] as $top):?>
                    <li class="dropdown">
                        <a href="<?php echo yii\helpers\Url::to(['product/index','cateid'=>$top['cateid']]);?>" <?php if(!empty($top['_son'])):?> class="dropdown-toggle" data-hover="dropdown" data-toggle="dropdown" <?php endif;?>><?php echo $top['title'];?></a>
<?php if(!empty($top['_son'])):?>
                        <ul class="dropdown-menu">
                            <li><div class="yamm-content">
    <div class="row">
        <?php 
            $count = count($top['_son']);
            if($count){
                $class = 'col-sm-'.floor(12/$count);
            }
        ?>
       <?php foreach($top['_son'] as $second):?>
       <div class="col-xs-12 <?php echo $class;?>">
            <h2><a href="<?php echo yii\helpers\Url::to(['product/index','cateid'=>$second['cateid']]);?>"><?php echo $second['title'];?></h2></a>
            <?php if(!empty($second['_son'])):?>
            <ul>
                <?php foreach($second['_son'] as $third):?>
                <li><a href="<?php echo yii\helpers\Url::to(['product/index','cateid'=>$third['cateid']]);?>"><?php echo $third['title'];?></a></li>
                <?php endforeach;?> 
            </ul>
            <?php endif;?> 
        </div><!-- /.col -->
